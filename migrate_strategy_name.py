...
# CHANGE THESE TWO paths to match your app.py constants / files on disk
QUALIFIED_FILE = "filtered_value_bets_qualified.json"
RESULTS_FILE   = "filtered_value_bets_results.json"
...
def migrate_qualified():
    ...
    for item in qualified:
        # rename the stored strategy name field (different projects sometimes use different keys)
        ...
        # bet_key format in your app: fixture_id:market:strategy_name:bookmaker
        bk = item.get("bet_key")
...
def migrate_results():
    ...
    for bet_key, payload in results.items():
        ...
        # If a collision happens, prefer whatever is already present under the new key
        if new_key not in new_results:
            # also update any embedded strategy name if present
